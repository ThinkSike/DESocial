...
import { getCampusLocationById } from '@/data/campusLocations';
...
export function CommunityFeed({ posts, selectedCommunity }: CommunityFeedProps) {
  ...
  const [selectedEvent, setSelectedEvent] = useState<{post: Post, location: any} | null>(null);
  ...
  const handleEventLocationPress = (locationId: string, post: Post) => {
    if (locationId) {
      const location = getCampusLocationById(locationId);
      if (location) {
        setSelectedEvent({ post, location });
    ...
    <View key={post.id} style={[styles.postCard, { 
      ...
      <View style={styles.postContent}>
        ...
        {post.event && (
          <TouchableOpacity 
            ...
            onPress={() => handleEventLocationPress(post.event!.locationId, post)}
          >
      ...
      <View style={[styles.postActions, { borderTopColor: colors.border }]}>
        ...
        <TouchableOpacity style={styles.actionButton}>
          ...
          <Text style={[styles.actionText, { color: colors.textSecondary }]}>{post.engagement.reposts}</Text>
        </TouchableOpacity>
        
        <TouchableOpacity style={styles.actionButton}>
          <Ionicons name="share-outline" size={20} color={colors.textSecondary} />
          <Text style={[styles.actionText, { color: colors.textSecondary }]}>{post.engagement.shares}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
